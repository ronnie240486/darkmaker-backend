...
/**
 * Detects a valid font path on the system to prevent drawtext from crashing.
 */
    ...
    try {
        ...
        for (let i = 0; i < visualFiles.length; i++) {
            ...
            await new Promise((resolve, reject) => {
                ...
                // Input 1: The Audio (Narration or Generated Silence)
                if (audio) {
                    cmd.input(audio.path);
                } else {
                    // Generate internal high-quality silence
                ...
                // Add Professional Subtitles only if a system font is available to prevent crashes
                if (text && text.trim().length > 0 && SYSTEM_FONT) {
                    const cleanText = sanitizeForFFmpeg(text);
                    const fontSize = Math.floor(targetH * 0.045);
                    const boxMargin = Math.floor(targetH * 0.1);
                    videoFilters.push(`drawtext=fontfile='${SYSTEM_FONT}':text='${cleanText}':fontcolor=white:fontsize=${fontSize}:box=1:boxcolor=black@0.5:boxborderw=20:line_spacing=10:x=(w-text_w)/2:y=h-th-${boxMargin}:fix_bounds=1`);
                } else if (text && text.trim().length > 0 && !SYSTEM_FONT) {
                    console.warn("Skipping drawtext: No system font file found.");
                }
