...
app.post('/ia-turbo', upload.fields([{ name: 'visuals' }, { name: 'audios' }, { name: 'bgMusic' }]), async (req, res) => {
    ...
    try {
        ...
        // 1. Processar cada cena individualmente com efeitos e legendas
        for (let i = 0; i < visualFiles.length; i++) {
            const visual = visualFiles[i];
            const audio = audioFiles[i];
            const text = narrations[i] || '';
            const segPath = path.join(UPLOAD_DIR, `s_${i}_${Date.now()}.mp4`);
            const isImg = /\.(jpg|jpeg|png|webp)$/i.test(visual.originalname);
            ...
            await new Promise((resolve, reject) => {
                ...
                if (isImg) {
                    ...
                    vFilters.push(`scale=8000:-1,zoompan=z='min(zoom+0.0015,1.5)':d=180:s=${targetW}x${targetH}:fps=30`);
                } else {
                ...
                if (text && fs.existsSync(FONT_PATH)) {
                    ...
                    vFilters.push(`drawtext=fontfile='${FONT_PATH.replace(/\\/g, '/')}:text='${clean}':fontcolor=white:fontsize=${fontSize}:box=1:boxcolor=black@0.6:boxborderw=20:x=(w-text_w)/2:y=${yPos}`);
                }
                ...
                .map('v_out').map('a_out')
                .outputOptions(['-c:v libx264', '-preset ultrafast', '-crf 22', '-shortest'])
                .save(segPath)
        ...
        if (bgMusicFile) {
            // Mixar narração (volume 1.0) com música de fundo (volume 0.15)
            complexFilter += `[a_narration]volume=1.0[a1];[${segments.length}:a]volume=0.15,dynaudnorm[a2];[a1][a2]amix=inputs=2:duration=first[a_final]`;
        } else {
            complexFilter += `[a_narration]copy[a_final]`;
        }

        finalCmd.complexFilter(complexFilter)
            .map('[v_raw]').map('[a_final]')
            .outputOptions(['-c:v libx264', '-preset fast', '-crf 20', '-c:a aac', '-b:a 192k', '-y'])
            .save(outputPath)
            .on('end', () => {
                ...
                setTimeout(() => {
                    ...
                    [...visualFiles, ...audioFiles, bgMusicFile].filter(Boolean).forEach(f => fs.unlink(f.path, () => {}));
                }, 30000);
            })
