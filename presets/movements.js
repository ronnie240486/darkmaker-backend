...
// Totalmente compatível com FFmpeg 6 — SEM ERROS
...
export function getMovementFilter(moveId, durationSec = 5, targetW = 1280, targetH = 720) {
    ...
    // Pré-processamento (garante proporção e qualidade)
    ...
    // Tempo normalizado em zoompan
    const tz = `(on/${totalFrames})`;

    // Tempo normalizado para filtros (rotate, boxblur, noise)
    ...
    const moves = {
        ...
        'zoom-in':  `zoompan=z='1.0+(0.5*${tz})':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)'${zdur}`,
        'zoom-out': `zoompan=z='1.5-(0.5*${tz})':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)'${zdur}`,

        'mov-zoom-crash-in':  `zoompan=z='1.0+3*${tz}*${tz}*${tz}':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)'${zdur}`,
        'mov-zoom-crash-out': `zoompan=z='4-3*${tz}':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)'${zdur}`,
        ...
        // --------------------------------------------------
        // EFEITOS COM ROTATE/BOXBLUR (CORRIGIDO)
        // --------------------------------------------------

        'mov-zoom-twist-in': `zoompan=z='1.0+(0.5*${tz})'${zdur},rotate=angle='(PI/12)*${tg}':fillcolor=black:ow=${targetW}:oh=${targetH}`,

        // BLUR (Compatível FFmpeg 6)
        'mov-blur-in':    `zoompan=z=1${zdur},boxblur='20*(1-${tg})':1`,
        'mov-blur-out':   `zoompan=z=1${zdur},boxblur='20*${tg}':1`,
        'mov-blur-pulse': `zoompan=z=1${zdur},boxblur='6+4*abs(sin(${tg}*PI*2))':1`,

        // Tilt-shift simulado
        'mov-tilt-shift': `zoompan=z='1.0+(0.1*${tz})'${zdur},boxblur='2':1,vignette=a=PI/5`,
